// methNewtone.cpp: определ€ет точку входа дл€ консольного приложени€.
//
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	//коэффиценты уравнени€ в пор€дке убывни€ степени, включа€ нули и перед нулевой степенью
...
void findAndSepRoots(int n, double* koefs, double eps) {
	...
	cout << "ћодули всех корней лежат между " << r << " и " << R << endl;
	int k = 2;
...
double* solveSqrEq(int n, double* koefs) {
	if (n == 2) {
		...
		cout << "ƒанное уравнение не €вл€етс€ квадратным\n";
		return NULL;
	}
}
...
bool NewtConditions(int n, double* koefs, double* segment) {
	if (n == 3) {
		...
		//втора€ производна€ у кубической функции - пр€ма€
	...
	cout << "\n ƒанное уравнение не €вл€етс€ кубическим, поэтому непровер€емы услови€ сходимости/n";
	return false;
}
...
void methNewtone(int n, double* koefs, double* segment, double eps) {
	...
	double xCur;
	double xNext;
	//т.к. перва€ и втора€ производна€ на отрезке сохран€ют знак, то провер€ем в точке из отрезка
	...
	cout << " орень лежит р€дом с " << setprecision(8) << xNext << endl;
}
